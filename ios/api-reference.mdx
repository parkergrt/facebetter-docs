---
title: "API Reference"
description: "iOS Objective-C API for Facebetter SDK 2.0"
---
...
iOS and macOS share this Objective-C API. iOS-only: `+[FBImageFrame createWithUIImage:]`. Minimum OS: **iOS 12.0**, architecture **arm64**.
...
| `externalContext` | `BOOL` | Default `NO`. `YES`: use the caller’s OpenGL ES context; SDK does not create one |

**Auth priority:** if `licenseToken` is non-empty, validate that token locally and skip `/facebetter/v2/auth`. Otherwise use `appId` + `appKey` for online auth. Bind the app **Bundle ID** in the Dashboard.
...
No register-by-id. Path or bytes; GPU resources are created on the next `processImage:` (GL thread / external context).
...
Reads `imageFrame.type` (`FBFrameTypeImage` or `FBFrameTypeVideo`). Output format matches input. Returns `nil` on failure.
...
| `createWithTexture:width:height:stride:` | `GL_TEXTURE_2D` in the current context; `stride` usually `width * 4` |
| `createWithUIImage:` | **iOS only** |
...
| `mirror:` | `"horizontal"` / `"vertical"` / `"both"` (case-insensitive); mutates pixels |
| `setMirror:` | Flag for `processImage:` (avoids extra conversions). `nil` / `@""` clears |
...
`width`, `height`, `stride`, `size`, `type`. Methods: `data`, `format`, `texture` (`0` if not a texture). YUV: `dataY` / `dataU` / `dataV` / `dataUV`, `strideY` / `strideU` / `strideV` / `strideUV` (`NULL` / `0` if not YUV).
