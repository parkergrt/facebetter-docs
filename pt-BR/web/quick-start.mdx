...
O SDK carrega os arquivos de runtime durante `init()`. Você **não** precisa copiar `resource.fbd` para `public/`. Opcional: `init({ onProgress })` informa o percentual de download. Adesivos 3D precisam do pacote extra `resource_3d.fbd` — veja [Pacotes de recursos opcionais](/pt-BR/intro/resource-packs).
