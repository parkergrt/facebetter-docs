...
Skin / makeup / body-reshape intensities are `[0.0, 1.0]`. `0` is off. Face reshape is **`[-1.0, 1.0]`**. Full enum lists: [Parameter enums](/intro/makeup).
...
Range **`[0.0, 1.0]`**. `0` is off. Place `resource_body.fbd` next to `resource.fbd` or call `AddResourcePack`. See [Parameter enums](/intro/makeup) and [Optional resource packs](/intro/resource-packs).
...
LUT / sticker textures are created on the next `ProcessImage` call (the active GL thread). For 3D stickers see [Optional resource packs](/intro/resource-packs).
