...
`FBEngine.create`로 인스턴스 하나를 만듭니다. 프로세스 전역 싱글톤이 아닙니다.
...
`path`와 `data`는 상호 배타적이며, 정확히 하나가 필요합니다. 3D 스티커는 [선택 리소스 팩](/ko/intro/resource-packs)을 참고하세요.
...
- `FBMirror` — `none`, `horizontal`, `vertical`, `both`(라이브 텍스처 경로에서는 보통 사용하지 않음)
- `FBFace` / `FBFaceList` / `FBRect` — `enableLandmarks: true`일 때의 랜드마크 뷰(`kLandmarkCount` = 111)

## 네이티브 텍스처 (Dart 아님)

`externalContext: true`로 `create`한 뒤, **벤더 SDK GL 스레드**에서 호출하세요.
