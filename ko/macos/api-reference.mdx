...
iOS와 macOS는 이 Objective-C API를 공유합니다. macOS 전용: `+[FBImageFrame createWithNSImage:]`. 최소 OS: **macOS 10.15**, 아키텍처 **x86_64**와 **arm64**. 정지 이미지에는 AppKit(`NSImage`)을 사용하세요.
...
| `externalContext` | `BOOL` | 기본 `NO`. `YES`: 호출 측 OpenGL 컨텍스트를 사용 |

**인증 우선순위:** `licenseToken`이 비어 있지 않으면 해당 토큰을 로컬에서 검증하고 `/facebetter/v2/auth`를 건너뜁니다. 그렇지 않으면 `appId` + `appKey`로 온라인 인증합니다. Dashboard에서 앱 **Bundle ID**를 바인딩하세요. 번들이 없는 커맨드라인 도구는 프로세스 이름을 바인딩하세요.
