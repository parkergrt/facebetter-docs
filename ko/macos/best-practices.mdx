...
사진을 열면 전체 `NSImage`를 유지하고 미리보기 프레임을 **한 장** 만드세요(긴 변 약 1280–1440, `createWithNSImage:`). 파라미터 조절은 미리보기(`FBFrameTypeImage`)를 처리하고, 내보내기는 같은 setter로 전체 해상도를 한 번 더 처리합니다. 출력 크기는 입력과 같습니다. 카메라 미리보기는 캡처 해상도 + `FBFrameTypeVideo`입니다.
