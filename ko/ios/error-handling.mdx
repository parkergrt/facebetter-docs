---
title: "오류 처리"
description: "iOS Facebetter SDK 2.0 오류 처리"
---
...
프레임을 검증하고 `type`을 설정한 뒤, `nil` 반환을 `FBErrorCode_Process`로 다루세요.
...
엔진을 만들기 **전에** 로그를 켜세요. 파일을 쓰면 로그 디렉터리를 만드세요.
...
- **`-2` 미초기화**: 실패한 `createEngineWithConfig:` 이후 setter 호출(인스턴스가 `nil`이므로 메시지를 보내지 마세요).
- **`-3` 라이선스**: 잘못된 `appId` / `appKey`, 바인딩되지 않은 Bundle ID, 만료된 토큰, 또는 온라인 인증용 네트워크 없음. [인증 및 라이선스](/ko/intro/license)를 참고하세요.
- **`-5` I/O**: 없는 필터 / 스티커 / 배경 파일, 또는 읽을 수 없는 샌드박스 경로.
- **외부 GL**: `externalContext = YES`이면 같은 OpenGL ES 스레드에서 생성하고 처리하세요. 그렇지 않으면 `processImage:`가 `nil`을 반환할 수 있습니다.
