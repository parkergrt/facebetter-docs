...
|------|--------------|-------------|
| Token | `licenseToken` | Your server already issued a license token or `{token}` JSON |
| Proxy | `authProxyUrl` | The SDK posts an auth request to **your** server; the server holds `app_id` / `app_key` and calls the Facebetter auth API |
| Custom | `fetchAuthResponse` | You implement `async (challenge) => string \| object`. Local debug only: `createDirectAuthFetcher({ appId, appKey })` |
...
// A. Server already returned a v2 token
const config = new EngineConfig({
  licenseToken: await fetch('/api/facebetter/auth', { method: 'POST' }).then((r) => r.text()),
});

// B. Production: proxy the SDK auth request (recommended)
const config = new EngineConfig({
  authProxyUrl: '/api/facebetter/auth',
});

// C. Local debug only — keys appear in the frontend
const config = new EngineConfig({
