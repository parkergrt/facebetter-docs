...
description: "Face reshape, body reshape, makeup, whitening, and smoothing enums for Facebetter SDK 2.0"
---
...
<Note>
Face reshape is available from **Pro**. Free can try it with a watermark. See [Plans & Features](./pricing).
</Note>
...
## Body reshape (`BodyReshape`)

Used with `SetBodyReshape(param, intensity)`. Range is **`[0.0, 1.0]`**. `0` is off. Unlike face reshape, these values are not bipolar.

<Note>
Body reshape requires **Pro+**. Free can try it with a watermark. See [Plans & Features](./pricing).
</Note>

Requires the optional pack `resource_body.fbd` except `LegStretch` and `TorsoLong`. Register it with `AddResourcePack` / `addResourcePack` **before** pose-based `SetBodyReshape` / `setBodyReshape`. See [Optional resource packs](./resource-packs).

| Value | Name | Meaning |
| --- | --- | --- |
| 0 | `BodySlim` | Slim the whole body |
| 1 | `WaistSlim` | Slim the waist |
| 2 | `LegSlim` | Slim the legs |
| 3 | `ShoulderSlim` | Slim the shoulders |
| 4 | `ArmSlim` | Slim the arms |
| 5 | `LegLong` | Lengthen the legs along the skeleton |
| 6 | `BustEnhance` | Enhance the bust |
| 7 | `LegStretch` | Stretch below the waist (face only; no `resource_body.fbd`) |
| 8 | `TorsoLong` | Lengthen the torso chin-to-waist (face only; no `resource_body.fbd`) |

Java names are `SNAKE_CASE` (`BODY_SLIM`, `WAIST_SLIM`, …). Objective-C uses `FBBodyReshape_BodySlim`. Dart uses `FBBodyReshape.bodySlim`.
