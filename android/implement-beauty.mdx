...
Range **`[0.0, 1.0]`**. `0` is off. Register `resource_body.fbd` with `addResourcePack` first (not in the AAR). See [Parameter enums](/intro/makeup) and [Optional resource packs](/intro/resource-packs).
