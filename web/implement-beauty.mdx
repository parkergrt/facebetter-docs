...
| Field | Purpose |
| --- | --- |
| `licenseToken` | License token string, or `{token}` JSON |
| `authProxyUrl` | Production: POST the SDK auth request to **your** server; the server holds `app_id` / `app_key` |
| `fetchAuthResponse` | `async (challenge) => string \| object`. Local debug: `createDirectAuthFetcher({ appId, appKey })` |
| `resourcePath` | URL of `resource.fbd`. Default `/resource.fbd` |

Pick **one** auth mode. Web does **not** support offline `.lic`.
