...
Range is **`[0.0, 1.0]`**. `0` is off. Fetch `resource_body.fbd` and call `addResourcePack` after `init()` first. Full list: [Parameter enums](/intro/makeup). Packs: [Optional resource packs](/intro/resource-packs).
...
3D stickers need the optional `resource_3d.fbd` pack (not in the Web SDK). See [Optional resource packs](/intro/resource-packs). Fetch bytes and register them after `init()`:
