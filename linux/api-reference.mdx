---
title: "API Reference"
description: "Facebetter SDK 2.0 C++ API for Linux"
---

<Note>
SDK **2.0.0**. Namespace `facebetter`. Same public C++ API as [Windows](/windows/api-reference). Headers: `facebetter/*.h`. Enums: [Parameter enums](/intro/makeup). Auth: [Auth & License](/intro/license).
</Note>

## Linux notes

- Ship `libfacebetter.so`. Set `LD_LIBRARY_PATH`, rpath, or `ldconfig`.
- Build the SDK with `./scripts/build_linux.sh` (output `build/linux/release`) or CMake.
- `resource_path` must be the **`resource.fbd` file**.
- `SetRenderView` is not available.
- `external_context = true` uses the calling thread’s current GL context (GLFW / EGL / OSMesa).
...
| Field | Description |
| --- | --- |
| `app_id` / `app_key` | Native online v2 auth |
| `resource_path` | Path to `resource.fbd` (file, not directory) |
| `license_token` | token / `{token}` JSON / `.lic` — non-empty takes priority |
| `external_context` | `false`: SDK-managed OpenGL; `true`: caller context |
...
Full `Reshape` list: [Parameter enums](/intro/makeup).
...
Full `BodyReshape` list: [Parameter enums](/intro/makeup). Requires `resource_body.fbd`. See [Optional resource packs](/intro/resource-packs).
...
Lipstick is Color only; pupil is Color only; contour is Style only. Values: [Parameter enums](/intro/makeup).
...
Path or encoded `.fbd` bytes. No register / ID API. Takes effect on the next `ProcessImage`.
...
Set `frame->type` to `FrameType::Image` or `FrameType::Video`.
...
**Accessors:** `Width/Height/Stride/Size/GetFormat/Data`, YUV planes, `Texture()`, `Buffer()`. Field: `FrameType type`.
...
Serialize `ProcessImage` on one thread. Pixel `Data()` pointers are read-only.
