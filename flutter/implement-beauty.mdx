...
<Note>
SDK **2.0.0**. Class: **`FBEngine`**. No singleton. Auth: [Auth & License](/intro/license). Enums: [Parameter enums](/intro/makeup).
</Note>

Flutter **only sets parameters**. Live video frames never enter Dart. Third-party RTC SDKs feed textures on the native GL thread — see [Third-party Integration](/flutter/third-party-integration).
...
| `enableLandmarks` | `false` | Engine-level; cannot change later. Flutter has no landmark callback — keep `false` |
| `externalContext` | `true` | `true` for third-party live preprocess; `false` for photo file APIs |
...
Body reshape **`[0.0, 1.0]`**. Register `resource_body.fbd` first. See [Parameter enums](/intro/makeup) and [Optional resource packs](/intro/resource-packs):
...
3D stickers need the optional `resource_3d.fbd` pack (not in the plugin). See [Optional resource packs](/intro/resource-packs).
...
Use a **separate** engine with `externalContext: false`. Do **not** call these on a live preprocess engine, and do **not** process meeting / push video from Dart.
...
Stop the vendor’s custom preprocess first, then:
...
- [Third-party Integration](/flutter/third-party-integration)
