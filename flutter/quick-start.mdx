...
<Note>
This page matches SDK **2.0.0**. The engine class is **`FBEngine`** (not a singleton). Auth: [Auth & License](/intro/license). Enums: [Parameter enums](/intro/makeup).
</Note>
...
Copy `appId` / `appKey`, or pass a `licenseToken` (server-issued short-lived token, or an offline `.lic` obtained via sales). See [Subscription](/intro/enable-service) and [Auth & License](/intro/license).
...
<Tip>
`externalContext: true` (default) reuses the caller GL context — required for third-party live preprocess. Still-image APIs need a **second** engine with `externalContext: false`. Do not mix the two on one instance.
</Tip>
...
- [Third-party Integration](/flutter/third-party-integration) — native `FacebetterPlugin.processTexture`
