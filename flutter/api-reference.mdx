...
<Note>
This page matches SDK **2.0.0**. Class: **`FBEngine`**. Makeup / reshape enums: [Parameter enums](/intro/makeup). Auth: [License & Auth](/intro/license).
</Note>
...
- `FBMirror` — `none`, `horizontal`, `vertical`, `both` (not used on the TRTC texture path)
...
Returns `0` on failure. Full snippets: [TRTC integration](/flutter/trtc-integration).
