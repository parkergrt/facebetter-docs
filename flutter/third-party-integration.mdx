...
| Live engines | At most one `externalContext: true` engine; a second `create` throws until you `dispose()` the first |
...
**Do not:** push pixels or texture ids from Dart every frame; call `processTexture` off the GL thread; create a second `externalContext: true` engine before disposing the first.
