...
description: "API Objective-C de Facebetter SDK 2.0 para iOS"
---
...
iOS y macOS comparten esta API Objective-C. Solo iOS: `+[FBImageFrame createWithUIImage:]`. SO mínimo: **iOS 12.0**, arquitectura **arm64**.
...
| `externalContext` | `BOOL` | Predeterminado `NO`. `YES`: usa el contexto OpenGL ES del llamador; el SDK no crea uno |

**Prioridad de autenticación:** si `licenseToken` no está vacío, valida ese token en local y omite `/facebetter/v2/auth`. En caso contrario usa `appId` + `appKey` para autenticación en línea. Vincula el **Bundle ID** de la app en la Consola.
...
No hay registro por ID. Ruta o bytes; los recursos GPU se crean en el siguiente `processImage:` (hilo GL / contexto externo).
...
Lee `imageFrame.type` (`FBFrameTypeImage` o `FBFrameTypeVideo`). El formato de salida coincide con el de entrada. Devuelve `nil` si falla.
...
| `createWithTexture:width:height:stride:` | `GL_TEXTURE_2D` en el contexto actual; `stride` suele ser `width * 4` |
| `createWithUIImage:` | **Solo iOS** |
...
| `mirror:` | `"horizontal"` / `"vertical"` / `"both"` (sin distinguir mayúsculas); muta los píxeles |
| `setMirror:` | Marca para `processImage:` (evita conversiones extra). `nil` / `@""` la borra |
| `convert:` | Devuelve un fotograma nuevo en el `FBImageFormat` de destino |
| `toFile:quality:` | quality `0`–`100` |
| `toFile:` | quality `90` |

### Propiedades y accesores

`width`, `height`, `stride`, `size`, `type`. Métodos: `data`, `format`, `texture` (`0` si no es textura). YUV: `dataY` / `dataU` / `dataV` / `dataUV`, `strideY` / `strideU` / `strideV` / `strideUV` (`NULL` / `0` si no es YUV).
