...
<Note>
SDK **2.0.0**。类名：**`FBEngine`**。没有单例。鉴权：[授权与许可证](/zh/intro/license)。枚举：[参数枚举](/zh/intro/makeup)。
</Note>

Flutter **只设置美颜参数**。实时视频帧不进 Dart。TRTC / 推流 SDK 在原生 GL 线程喂纹理 — 见 [TRTC 对接](/zh/flutter/trtc-integration)。
...
| `externalContext` | `true` | 接 TRTC 用 `true`；静图文件接口用 `false` |
...
使用**另一台**引擎，且 `externalContext: false`。不要在 TRTC 引擎上调用这些接口，也不要从 Dart 逐帧处理 TRTC 视频。
...
- [TRTC 对接](/zh/flutter/trtc-integration)
