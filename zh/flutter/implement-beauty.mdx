...
| `enableLandmarks` | `false` | 引擎级开关，创建后不能改。Flutter 没有关键点回调，请保持 `false` |
...
美体 **`[0.0, 1.0]`**。先登记 `resource_body.fbd`。见 [参数枚举](/zh/intro/makeup) 与 [可选资源包](/zh/intro/resource-packs)：
...
3D 贴纸需要可选包 `resource_3d.fbd`（不在插件内）。见 [可选资源包](/zh/intro/resource-packs)。
