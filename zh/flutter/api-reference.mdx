...
用 `FBEngine.create` 创建实例。不是进程级单例。同时只能有一台 `externalContext: true` 的引擎绑定到 `processTexture`。
...
`setBodyReshape(FBBodyReshape param, double intensity)` — **`[0.0, 1.0]`**。Dart 名：`FBBodyReshape.bodySlim` … `torsoLong`。需先 `addResourcePack` 加载 `resource_body.fbd`。表格见 [参数枚举](/zh/intro/makeup)，资源包见 [可选资源包](/zh/intro/resource-packs)。
...
`path` 与 `data` 互斥，必须且只能提供一个。3D 贴纸与美体见 [可选资源包](/zh/intro/resource-packs)。
...
以 `externalContext: true` `create` 后，在 **对方 SDK 的 GL 线程**上调用。同时只能绑定一台实时引擎；未 `dispose` 前再 `create(externalContext: true)` 会抛错。静图引擎（`externalContext: false`）不会占用这个槽。
