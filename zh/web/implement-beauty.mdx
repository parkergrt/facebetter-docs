...
引擎运行时文件在 `init()` 时由 SDK 加载，不必在站点根路径放 `resource.fbd`。可选下载进度：`init({ onProgress })`。引擎不会去换 token。Web **不支持**离线 `.lic`。
...
范围是 **`[0.0, 1.0]`**。`0` 关闭。先在 `init()` 之后拉取 `resource_body.fbd` 并 `addResourcePack`。完整列表：[参数枚举](/zh/intro/makeup)。资源包：[可选资源包](/zh/intro/resource-packs)。
...
3D 贴纸需要可选包 `resource_3d.fbd`（不在 Web SDK 内）。见 [可选资源包](/zh/intro/resource-packs)。`init()` 之后用字节登记：
