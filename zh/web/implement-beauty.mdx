...
| 字段 | 用途 |
| --- | --- |
| `licenseToken` | 许可证 token，或 `{token}` JSON |
| `authProxyUrl` | 生产环境：把 SDK 鉴权请求 POST 到**你的**服务器；由服务器持有 `app_id` / `app_key` |
| `fetchAuthResponse` | `async (challenge) => string \| object`。本地调试：`createDirectAuthFetcher({ appId, appKey })` |
| `resourcePath` | `resource.fbd` 的 URL，默认 `/resource.fbd` |

鉴权三选一。Web **不支持**离线 `.lic`。
