---
title: "错误处理"
description: "macOS Facebetter SDK 2.0 错误处理"
---
...
if (self.engine == nil) {
  NSLog(@"创建引擎失败（检查授权、Bundle ID、网络和日志）");
  return;
}
...
校验帧、设置 `type`，将 `nil` 视为处理失败（`FBErrorCode_Process`）。macOS 可处理更大静图，但仍应拒绝零尺寸缓冲。
...
`createWithNSImage:` 在 `NSImage` 没有位图表示时返回 `nil`。

## 日志

在**创建引擎之前**打开日志。写文件时先创建目录。沙盒 Mac 应用应写到容器内（例如 Application Support），不要写任意路径。
...
- **App Sandbox**：在线 `appId` / `appKey` 需要出站网络。用户选择的文件需要 security-scoped bookmark（`startAccessingSecurityScopedResource`）。
- **`-5` I/O**：`.fbd` / 背景图在沙盒外，或未 start bookmark。
- **命令行工具**：没有 Bundle ID 时，请在控制台绑定进程名。
- **外部 GL**：`externalContext = YES` 时必须在同一 OpenGL 线程创建并处理。

## 常见原因

- **`-1` 参数无效**：空 `.fbd` 路径、`NSData` 为 nil、强度越界、配置为 nil。
- **`-2` 未初始化**：`createEngineWithConfig:` 失败后实例为 `nil`。
- **`-3` 授权**：凭证错误、未绑定 Bundle ID、token 过期，或沙盒拦住授权请求。见 [授权与许可](/zh/intro/license)。
