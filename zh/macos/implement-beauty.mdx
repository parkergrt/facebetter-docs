...
绑定 macOS Bundle ID，然后传入 `licenseToken` 或 `appId` + `appKey`。见 [获取 AppID 和 AppKey](/zh/intro/enable-service#获取-appid-和-appkey) 与 [授权与许可](/zh/intro/license)。
...
接入 TRTC / 声网 / LiveKit：见 [第三方对接](/zh/macos/third-party-integration)。
...
- [第三方对接](/zh/macos/third-party-integration)
