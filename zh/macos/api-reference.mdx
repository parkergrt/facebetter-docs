---
title: "API 参考"
description: "Facebetter SDK 2.0 macOS Objective-C API"
---
...
iOS 与 macOS 共用同一套 Objective-C API。macOS 独有：`+[FBImageFrame createWithNSImage:]`。最低系统：**macOS 10.15**，架构 **x86_64** 与 **arm64**。静图使用 AppKit（`NSImage`）。
...
| `externalContext` | `BOOL` | 默认 `NO`。`YES`：使用调用方的 OpenGL 上下文 |

**授权优先级：** `licenseToken` 非空则本地校验，跳过 `/facebetter/v2/auth`；否则用 `appId` + `appKey` 在线授权。请在控制台绑定应用 **Bundle ID**。没有 bundle 的命令行工具请绑定进程名。
...
没有按 ID 注册。传路径或字节；GPU 资源在下一次 `processImage:` 创建。
...
读取 `imageFrame.type`。输出格式与输入一致。失败返回 `nil`。
...
| `createWithBGRA:width:height:stride:` | AppKit 位图常用 |
...
| `createWithTexture:width:height:stride:` | 当前上下文中的 `GL_TEXTURE_2D` |
| `createWithNSImage:` | **仅 macOS** |
...
| `mirror:` | `"horizontal"` / `"vertical"` / `"both"`；直接改像素 |
| `setMirror:` | 供 `processImage:` 使用的标记。`nil` / `@""` 清除 |
| `convert:` | 返回目标 `FBImageFormat` 的新帧 |
| `toFile:quality:` | 质量 `0`–`100` |
| `toFile:` | 质量 `90` |

访问器：`width`、`height`、`stride`、`size`、`type`、`data`、`format`、`texture`。YUV：`dataY` / `dataU` / `dataV` / `dataUV` 及对应 stride。
