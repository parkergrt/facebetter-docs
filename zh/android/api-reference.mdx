...
<Note>
本文档对应 SDK **2.0.0**。包名：`net.pixpark.facebetter`。美妆、整形、美白、磨皮预设见 [参数枚举](/zh/intro/makeup)。鉴权见 [授权与许可](/zh/intro/license)。
</Note>
...
**鉴权优先级：** `licenseToken` 非空时，SDK 在本地校验该 token。否则必须提供 `appId` + `appKey`，SDK 请求 `/facebetter/v2/auth`。详见 [授权与许可](/zh/intro/license)。
...
强度范围 **`[0.0, 1.0]`**。`0` 关闭。需先用 `addResourcePack` 加载 `resource_body.fbd`。见 [参数枚举](/zh/intro/makeup) 与 [可选资源包](/zh/intro/resource-packs)。
...
| `clearSticker` | 移除当前贴纸 |
| `addResourcePack` | 登记额外能力包（3D 贴纸前加载 `resource_3d.fbd`，美体前加载 `resource_body.fbd`） |
| `set3DSticker` | 从 `.fbd` 路径或字节应用 3D 贴纸 |
| `clear3DSticker` | 移除 3D 贴纸 |
