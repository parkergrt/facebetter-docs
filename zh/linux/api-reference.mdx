...
<Note>
SDK **2.0.0**。命名空间 `facebetter`。与 [Windows](/zh/windows/api-reference) 同一套公共 C++ API。头文件：`facebetter/*.h`。枚举见 [参数枚举](/zh/intro/makeup)。鉴权见 [授权与许可](/zh/intro/license)。
</Note>
...
完整 `BodyReshape` 列表见 [参数枚举](/zh/intro/makeup)。需要 `resource_body.fbd`，见 [可选资源包](/zh/intro/resource-packs)。
