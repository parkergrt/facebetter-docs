...
皮肤 / 美妆 / 美体强度为 `[0.0, 1.0]`，`0` 表示关闭。美型为 **`[-1.0, 1.0]`**。完整枚举见 [参数枚举](/zh/intro/makeup)。
...
范围 **`[0.0, 1.0]`**。`0` 关闭。把 `resource_body.fbd` 放在 `resource.fbd` 同目录，或调用 `AddResourcePack`。见 [参数枚举](/zh/intro/makeup) 与 [可选资源包](/zh/intro/resource-packs)。
...
LUT / 贴纸纹理在下一次 `ProcessImage`（当前 GL 线程）时创建。3D 贴纸见 [可选资源包](/zh/intro/resource-packs)。
