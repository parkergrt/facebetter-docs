...
<Note>
SDK **2.0.0**。命名空间 `facebetter`。头文件：`facebetter/beauty_effect_engine.h`、`beauty_params.h`、`image_frame.h`、`type_defines.h`。风格 / 色号 / 美型枚举见 [参数枚举](/zh/intro/makeup)。鉴权见 [许可证与鉴权](/zh/intro/license)。Linux 使用 **同一套** C++ API。
</Note>
