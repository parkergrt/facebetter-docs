...
`intensity` 为 **`[0.0, 1.0]`**。`0` 关闭。需要 `resource_body.fbd`（与 `resource.fbd` 同目录，或 `AddResourcePack`）。完整列表见 [参数枚举](/zh/intro/makeup)，资源包见 [可选资源包](/zh/intro/resource-packs)。
...
3D 贴纸或美体前请先加载对应可选包，见 [可选资源包](/zh/intro/resource-packs)。
