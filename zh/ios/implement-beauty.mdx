...
绑定 iOS Bundle ID，然后传入 `licenseToken` 或 `appId` + `appKey`。见 [获取 AppID 和 AppKey](/zh/intro/enable-service#获取-appid-和-appkey) 与 [授权与许可](/zh/intro/license)。
...
`setBodyReshape:intensity:` 范围为 **`[0.0, 1.0]`**。`0` 关闭。先用 `addResourcePack:` 登记 `resource_body.fbd`。参数：`FBBodyReshape_BodySlim` … `FBBodyReshape_TorsoLong`（`0`–`8`）。见 [参数枚举](/zh/intro/makeup) 与 [可选资源包](/zh/intro/resource-packs)。
...
3D 贴纸需要可选包 `resource_3d.fbd`（不在 framework 内）。见 [可选资源包](/zh/intro/resource-packs)：
...
接入 TRTC / 声网 / LiveKit：见 [第三方对接](/zh/ios/third-party-integration)。
...
- [第三方对接](/zh/ios/third-party-integration)
