...
<Note>
本文档对应 SDK **2.0.0**。预设含义见 [参数枚举](/zh/intro/makeup)。授权见 [授权与许可](/zh/intro/license)。
</Note>
...
`setBodyReshape:intensity:` — 强度 **`[0.0, 1.0]`**，`0` 关闭。需先用 `addResourcePack:` 加载 `resource_body.fbd`。`FBBodyReshape` 取值 `0`–`8`。见 [参数枚举](/zh/intro/makeup) 与 [可选资源包](/zh/intro/resource-packs)。
...
| `addResourcePack:` | 额外能力包路径（3D 贴纸前加载 `resource_3d.fbd`，美体前加载 `resource_body.fbd`） |
| `addResourcePackWithData:` | 额外包字节 |
| `set3DSticker:` | 3D 贴纸 `.fbd` 路径 |
| `set3DStickerWithData:` | 3D 贴纸字节 |
